...
import { useItineraryScreen, TOD_START_HOURS, QUICK_FILL_CATEGORIES, pickRandomActivity } from '@travyl/shared';
...
export default function ItinerarySection({ tripId }: { tripId: string }) {
  const { trip, isLoading, isEmpty } = useItineraryScreen(tripId);
  const {
  ...
  const [favorites, setFavorites] = useState<string[]>(() => {
    ...
    try { return JSON.parse(localStorage.getItem(`favorites-${tripId}`) || '[]'); } catch { return []; }
  });
  ...
  const persistActivitiesToDb = useCallback(async (newActivities: PersistActivity[], replaceDayDate?: string) => {
    ...
    try {
      ...
      const { data: tripData, error: readErr } = await supabase.from('trips').select('trip_context').eq('id', tripId).single();
      if (tripData?.trip_context) {
        ...
        const { error: updateErr } = await supabase.from('trips').update({ trip_context: { ...tripData.trip_context, itinerary, user_history: userHistory } }).eq('id', tripId);
      }

      // 2. Activity table — best effort (may fail for anonymous users due to RLS)
      const rows = newActivities.map(a => ({
        trip_id: tripId, user_id: null,
  ...
  const handleRegenerateDay = useCallback(async (dayIdx: number) => {
    ...
    try {
      ...
      const supabase = getSupabaseBrowser();
      await supabase.from('activity').delete().eq('trip_id', tripId).eq('starting_date', dayDateStr);
      await persistActivitiesToDb(added, dayDateStr);
    } catch (e) {
    } finally {
      setRegenerating(false);
    }
  }, [trip, days, addActivity, removeActivity, regenerating, tripId, persistActivitiesToDb]);
  ...
  }, [trip, days, regenerating, tripId, addActivity, removeActivity, persistActivitiesToDb]);
  ...
  const toggleFavorite = useCallback((itemId: string) => {
    setFavorites((prev) => {
      ...
      try { localStorage.setItem(`favorites-${tripId}`, JSON.stringify(next)); } catch {}
      return next;
    });
  }, [tripId]);
      ...
      {compactOpen && selectedDay && (
        ...
        <div ref={contentRef}>
          {isFirstDay && arrivalFlight && (
            <FlightSection flight={arrivalFlight} collapsed={allCollapsedOverride ?? undefined} onBookFlight={() => { window.location.href = `/trip/${tripId}/flights` }} />
          )}
          {/* Flight placeholder on first day — links to Flights tab */}
          {isFirstDay && !arrivalFlight && tripFlight && (
            <section className="mb-3.5">
              <a href={`/trip/${tripId}/flights`} className="block rounded-xl p-3 shadow-sm border border-blue-200/60 dark:border-blue-500/20 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-950/30 dark:to-indigo-950/20 hover:shadow-md transition-shadow group">
          ...
          {isFirstDay && firstHotel && (
            <section className="mb-3.5">
              <a href={`/trip/${tripId}/hotels`} className="block rounded-xl p-3 shadow-sm border border-amber-200/60 dark:border-amber-500/20 bg-gradient-to-r from-amber-50 to-orange-50 dark:from-amber-950/30 dark:to-orange-950/20 hover:shadow-md transition-shadow group">
                <div className="flex items-center gap-2.5">
          ...
          {isLastDay && (
            <>
              ...
              {returnFlight && <FlightSection flight={returnFlight} collapsed={allCollapsedOverride ?? undefined} onBookFlight={() => { window.location.href = `/trip/${tripId}/flights` }} />}
