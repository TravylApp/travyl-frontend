...
export function GlobalCommandPalette() {
  ...
  function selectPickerOption(value: string) {
    if (activePicker) {
      (activePicker as any).onSelect(value)
      exitPickerMode()
    }
  }
  ...
  function handleKeyDown(e: React.KeyboardEvent) {
    if (activePicker) {
      ...
      if (e.key === 'Escape') {
        ...
        setHighlightedIndex((prev) =>
          prev < (activePicker as any).options.length - 1 ? prev + 1 : prev
        )
        ...
        e.preventDefault()
        const opt = (activePicker as any).options[highlightedIndex]
    ...
    if (e.key === 'ArrowDown') {
      e.preventDefault()
      if (activePicker) {
        setHighlightedIndex((prev) => Math.min(prev + 1, (activePicker as any).options.length - 1))
      } else {
        setHighlightedIndex((prev) => {
      ...
      e.preventDefault()
      if (activePicker) {
        const option = (activePicker as any).options[highlightedIndex]
        if (option) {
          (activePicker as any).onSelect(option.value)
          setActivePicker(null)
          setQuery(savedQuery)
          setHighlightedIndex(0)
        }
        ...
        >
          ...
          >
            ...
            <div className="max-h-[360px] overflow-y-auto py-1">
              {/* ─── Picker mode ─── */}
              {activePicker ? (
                (activePicker as any).options.map((opt: any, i: number) => {
                  const isSelected = opt.label === (activePicker as any).currentValue
