...
export function usePackingList(tripId: string | undefined, userId: string | undefined, filterBy?: string) {
  ...
  return { items, itemsByCategory, orderedCategories, filteredItems, auditLog: auditQuery.data ?? [], progress, isLoading: itemsQuery.isLoading, error: itemsQuery.error, addItem, togglePacked, removeItem, claimItem, releaseItem, transferItem }
}
