...
// On web: use our own API proxy routes to avoid CORS (no prefix needed)
// On mobile: use the web app URL as proxy (has all API keys)
...
const API_URL = IS_WEB ? '' : (WEB_PROXY ?? process.env.EXPO_PUBLIC_RECOMMENDATION_API_URL ?? process.env.NEXT_PUBLIC_RECOMMENDATION_API_URL);
